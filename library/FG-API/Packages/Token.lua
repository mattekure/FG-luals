...
---@param tokena tokeninstance A tokeninstance object on the current image control.
---@param tokenb tokeninstance A tokeninstance object on the current image control.
---@return number # The distance between the two specified tokens in game system units
function Token.getDistanceBetween(tokena, tokenb) end
...
---@return tokeninstance|nil # A tokeninstance object representing the requested token, or nil if the container does not exist or the ID is not valid.
function Token.getToken(containernodename, containerid) end
...
function Token.getTokensWithinDistance(token, distance) end
...
function Token.onAdd(target) end
...
function Token.onClickDown(target, button, image) end
...
---@param image imagecontrol The imagecontrol object in which the token was clicked.
function Token.onClickRelease(target, button, image) end
...
function Token.onContainerChanged(target, oldcontainernode, oldcontainerid) end

---This handler is called just before a token is deleted.
---@param target tokeninstance The tokeninstance triggering the event.
function Token.onDelete(target) end

---This handler is called when the left mouse button is double-clicked on a token.
---@param target tokeninstance The tokeninstance triggering the event
---@param image imagecontrol The imagecontrol object in which the token was clicked.
function Token.onDoubleClick(target, image) end
...
function Token.onDrag(target, button, x, y, dragdata) end
...
---@param target tokeninstance The tokeninstance triggering the event
---@param dragdata dragdata A dragdata object containing the state of values connected to the mouse cursor as part of a drag and drop operation.
function Token.onDragEnd(target, dragdata) end
...
---@param dragdata dragdata A dragdata object containing the state of values connected to the mouse cursor as part of a drag and drop operation.
---@return boolean|nil # This function should return `true` if it handled the event and the processing of the event should be stopped. A value of `false` indicates that the default framework functionality for the this particular control should not be executed, but the processing should continue for the element below this control, if any. A return value of `nil` (or the absence of a `return` statement) indicates that the framework should continue handling the event normally.
function Token.onDragStart(target, button, x, y, dragdata) end
...
---@param target tokeninstance The tokeninstance triggering the event
---@param dragdata dragdata A dragdata object containing the state of values connected to the mouse cursor as part of a drag and drop operation.
function Token.onDrop(target, dragdata) end
...
function Token.onHover(target, state) end
...
function Token.onHoverUpdate(target, x, y) end

---This handler is called after a token is moved.
---@param target tokeninstance The tokeninstance triggering the event
function Token.onMove(target) end
...
function Token.onTargetUpdate(source, target, targeted) end
...
function Token.onWheel(target, notches) end
